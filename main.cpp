...
public:
    void Create(const sf::Vector2u& size, const int selectedNormalIndex)
    {
        ...
        setTexture(_texture);
        NWSprite.setTexture(_textureNx);
        NZSprite.setTexture(_textureNy);
        NYSprite.setTexture(_textureNz);
        NXSprite.setTexture(_textureNw);
...
int main()
{
    ...
    std::function<float(const sf::Vector2f&)> complexFunction = [&rFunctions](const sf::Vector2f& point) -> float {
        return ROr(rFunctions[0](point),ROr(rFunctions[1](point),ROr(rFunctions[2](point),ROr(rFunctions[3](point), rFunctions[4](point)))));
        };


    sf::FloatRect subSpace(-2.f, -2.f, 4.f, 4.f);

    static ImVec4 firstColor(0, 0, 0, 1);
    static ImVec4 secondColor(1, 1, 1, 1);

    rFuncSpriteNx.DrawRFunc(complexFunction, subSpace);
